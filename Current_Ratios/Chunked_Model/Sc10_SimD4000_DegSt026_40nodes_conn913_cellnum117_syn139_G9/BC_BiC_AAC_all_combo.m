...
f1 = figure;
for k = 1:1:64 %all combinations 4*4*4
...
for i = 4:4:64
    IPSC_all_on_BC_BiC_AAC_table = IPSC_all_on_BC_BiC_AAC_combo1(:,i-3:i);
    num = (1:4)';
...
Ratios_BC_BiC_AAC = array2table(reshape(temp_ratio_BC_BiC_AAC,[16,4]));
...
fig = uitable('Data',Ratios_BC_BiC_AAC{:,:},...
    ...
    'Position',[0, 0, 1, 1]);
%%
less_than_1 = sum(temp_ratio_BC_BiC_AAC <1)/64;
