...
for i = 1:1:14
    ...
    all_ipsc_on_BC = [all_ipsc_on_BC tot_cur_ipsc_on_BC];
end

% Find the peaks of the summed ipsc currents
peaks_all_PV_on_BC = [];
for k = 1:1:14
    [pks, locs] = findpeaks(all_ipsc_on_BC(:,k),'MinPeakDistance',4000); % peak detection
    temp_cur = all_ipsc_on_BC(:,k);
    allrows = (1:40000)';
    notpeak = setdiff(allrows,locs);
    for t = 1:1:numel(notpeak)
        element = notpeak(t,:);
    ...
    peaks_all = temp_cur;
    peaks_all_PV_on_BC = [peaks_all_PV_on_BC peaks_all];
end
...
ipsc_all_on_BC = [];
for i = 1:1:14 % number of PYR cells
    pks_ipsc_all = peaks_all_PV_on_BC(:,i);
    pks_ipsc_all(pks_ipsc_all == 0) = [];
    ipsc_all_mean = mean(pks_ipsc_all);
    ipsc_all_std = std(pks_ipsc_all);
    ipsc_all_on_BC = [ipsc_all_mean;ipsc_all_std];
    IPSC_all_on_BC = [IPSC_all_on_BC ipsc_all_on_BC];
end 

IPSC_all_on_BC = array2table(IPSC_all_on_BC);
IPSC_all_on_BC.Properties.VariableNames = {'BC1'...
    'BC2' 'BC3' 'BC4' 'BC5' 'BC6'...
    'BC7' 'BC8' 'BC9' 'BC10' 'BC11'...
    'BC12' 'BC13' 'BC14' 'BC15'};

IPSC_all_on_BC_mean = table2array(IPSC_all_on_BC(1,:));
IPSC_all_on_BC_std = table2array(IPSC_all_on_BC(2,:));
x = linspace(0,14,length(IPSC_all_on_BC_mean));
figure
scatter(x,IPSC_all_on_BC_mean,'black','filled');
xlabel('Individual Basket Cells','FontSize',13,'FontWeight','bold');
ylabel('Mean Peak IPSC','FontSize',13,'FontWeight','bold');
hold on;
errorbar(x,IPSC_all_on_BC_mean,IPSC_all_on_BC_std,'b','LineStyle','none')
